...
class SearchManager {
  ...
  final SplayTreeMap<String, Set<SearchEntry>> _queryMap =
  SplayTreeMap<String, Set<SearchEntry>>();
  ...
  factory SearchManager.init([List<SearchEntry> entries]) {
    if (entries != null) {
  ...
  _fill(List<SearchEntry> entries) {
    /// Sanity check.
    _queryMap.clear();
      ...
      for (int i = 0; i < len; i++) {
        for (int j = i + 1; j <= len; j++) {
          ...
          if (_queryMap.containsKey(substring)) {
            Set<SearchEntry> labels = _queryMap[substring];
            labels.add(e);
          } else {
  ...
  Set<SearchEntry> performSearch(String query) {
